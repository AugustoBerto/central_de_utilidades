...
const themeOptions = [
  ...
  },
  {
    value: 'system',
    label: 'Sistema',
    description: 'Acompanha a configuração do dispositivo.',
    icon: Laptop
  }
];
...
<template>
  <AppShell>
    <section class="max-w-3xl space-y-6">
        ...
        <div class="flex gap-3">
          ...
          <div>
            ...
            <p class="mt-1 text-sm text-muted">
              Escolha como o Painel de Utilidades deve aparecer.
            </p>
          </div>
        </div>
      ...
      <p
        v-if="notice"
        class="rounded-md border border-green-500/40 bg-green-500/10 p-3 text-sm text-green-600"
        role="status"
      >
        {{ notice }}
      </p>
      <p
        v-if="error"
        class="rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-500"
        role="alert"
      >
        {{ error }}
      </p>
      ...
      <p class="flex gap-2 text-xs text-muted">
        ...
        Sessões antigas podem não possuir detalhes do dispositivo; os campos serão preenchidos no próximo login.
      </p>
      <p class="flex gap-2 text-xs text-muted">
        <AlertTriangle :size="15" aria-hidden="true" />
        Em rede HTTP, use somente VPN ou uma rede privada confiável.
      </p>
    </section>
  </AppShell>
</template>
