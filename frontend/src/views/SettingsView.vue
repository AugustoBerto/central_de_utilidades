<script setup>
import { AlertTriangle, Laptop, LogOut, Monitor, Moon, RefreshCcw, ShieldCheck, Sun, User } from 'lucide-vue-next';
...
async function load() {
  ...
  try { sessions.value = await auth.getSessions(); }
  catch (caught) { error.value = caught.message; }
  finally { loading.value = false; }
}

async function logout() {
  loggingOut.value = true; error.value = '';
  try { await auth.logout(); await router.replace({ name: 'login' }); }
  catch (caught) { error.value = caught.message; loggingOut.value = false; }
}

async function revoke(session) {
  const accepted = await confirm({
    title: 'Revogar sessão?',
    message: `O acesso de “${session.deviceLabel}” será encerrado.`,
  ...
  try { await auth.revokeSession(session.id); notice.value = 'A sessão foi revogada.'; await load(); }
  catch (caught) { error.value = caught.message; }
    ...
    <section class="max-w-3xl space-y-6">
      <div class="rounded-lg border border-border bg-surface p-5">
        <div class="flex flex-wrap items-center gap-4">
          ...
          <div class="min-w-0 flex-1"><h2 class="font-semibold">Conta</h2><p class="mt-1 truncate text-sm text-muted">Usuário conectado: <span class="text-foreground">{{ auth.user?.username }}</span></p></div>
      ...
      <div class="rounded-lg border border-border bg-surface p-5">
        <div class="flex gap-3">
          ...
          <div><h2 class="font-semibold">Aparência</h2><p class="mt-1 text-sm text-muted">Escolha como o Painel de Utilidades deve aparecer.</p></div>
        </div>
        <div class="mt-5 grid gap-3 sm:grid-cols-3" role="radiogroup" aria-label="Tema da aplicação">
          <button
            ...
            :class="preference === option.value ? 'border-accent bg-accent/10' : 'border-border bg-canvas hover:border-accent/60'"
      ...
      <div class="rounded-lg border border-border bg-surface p-5">
        <div class="flex gap-3"><ShieldCheck class="mt-0.5 shrink-0 text-accent" :size="21" aria-hidden="true" /><div><h2 class="font-semibold">Sessões confiáveis</h2><p class="mt-1 text-sm text-muted">Este navegador permanece conectado por cookie persistente. Revogue acessos que não reconheça.</p></div></div>
        <div class="mt-5 flex flex-wrap gap-3"><AppButton variant="secondary" :disabled="loading || changing" @click="load"><RefreshCcw :size="16" aria-hidden="true" />Atualizar</AppButton><AppButton variant="danger" :disabled="loading || changing || sessions.length < 2" @click="revokeOthers">Revogar outras sessões</AppButton></div>
      </div>
      ...
      <ul v-else class="divide-y divide-border rounded-lg border border-border bg-surface">
        <li v-for="session in sessions" :key="session.id" class="flex items-center gap-4 p-4">
          ...
          <div class="min-w-0 flex-1"><p class="truncate font-medium">{{ session.deviceLabel }}<span v-if="session.current" class="ml-2 rounded-full bg-accent/15 px-2 py-0.5 text-xs text-accent">Atual</span></p><p class="mt-1 text-xs text-muted">Última atividade: {{ formatDate(session.lastUsedAt) }}</p></div>
          <AppButton v-if="!session.current" variant="danger" :disabled="changing" @click="revoke(session)">Revogar</AppButton>
        </li>
      </ul>
      <p class="flex gap-2 text-xs text-muted"><AlertTriangle :size="15" aria-hidden="true" />Em rede HTTP, use somente VPN ou uma rede privada confiável.</p>
    </section>
  </AppShell>
</template>
