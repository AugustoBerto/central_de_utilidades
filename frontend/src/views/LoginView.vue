...
async function submit() {
  ...
  try {
    ...
    await auth.login({
      ...
      deviceLabel: form.value.deviceLabel
    });
...
<template>
  <main
    class="grid min-h-screen place-items-center bg-canvas px-4 py-10 text-foreground"
  >
    <section
      class="w-full max-w-md rounded-lg border border-border bg-surface p-6 sm:p-8"
      aria-labelledby="login-title"
    >
      <div class="flex items-center gap-3 text-accent">
        <span class="rounded-md border border-border bg-elevated p-2"
          ><Activity :size="20" aria-hidden="true" /></span
        ...
        {{
          setupResult
            ...
            : isSetup
              ...
              : 'Este navegador permanecerá conectado até a sessão ser revogada ou expirar.'
        }}
      </p>
      <p
        v-if="error"
        class="mt-5 rounded-md border border-red-900 bg-red-950/40 p-3 text-sm text-red-200"
        role="alert"
      >
        {{ error }}
      </p>
      <div v-if="setupResult" class="mt-6 space-y-4 text-sm">
        <label class="block text-muted"
          >URI TOTP<textarea
            readonly
            class="mt-1 min-h-24 w-full rounded-md border border-border bg-canvas p-3 font-mono text-xs text-foreground"
            :value="setupResult.totpUri"
          />
        </label>
        <div>
        ...
        <AppButton class="w-full" @click="setupResult = null"
          >Continuar para o login</AppButton
        >
      </div>
        ...
        <label class="block text-sm"
          ...
          ><span class="relative mt-1 block"
            ><input
              ...
              class="w-full rounded-md border border-border bg-canvas px-3 py-2 pr-10"
      ...
      <div
        class="mt-6 flex gap-3 rounded-md border border-border bg-canvas p-4 text-sm text-muted"
      >
        <ShieldCheck
          :size="20"
