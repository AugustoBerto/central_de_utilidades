...
const storagePercent = computed(() => {
  const value = storageStatus.value?.usedPercent ?? 0;
...
const physicalDiskLimitsDrive = computed(() => {
  ...
  return Boolean(
    status &&
      status.physicalFreeBytes !== null &&
      status.physicalFreeBytes < status.quotaFreeBytes
  );
});
const currentFolderLabel = computed(() => currentFolder.value?.name ?? 'Meu Drive');
const resultLabel = computed(() => {
  if (loading.value) return 'Carregando…';
  const noun = total.value === 1 ? 'arquivo' : 'arquivos';
...
async function loadFolderContext() {
  ...
  try {
    ...
    const [children, detail] = await Promise.all([
      api(childrenPath),
      folderId === null
        ? Promise.resolve({ folder: null, path: [] })
...
async function refreshDirectory({ includeStatus = false } = {}) {
...
async function navigateToPath(folder) {
...
function retry(item) {
  ...
  queue.value = queue.value.filter((candidate) => candidate.id !== item.id);
  upload(item.file, item.targetFolderId === null ? null : {
...
onMounted(async () => {
  await Promise.all([loadStatus(), loadFolderContext(), loadFiles()]);
});
...
<template>
  <AppShell>
    <section
      class="relative min-h-[calc(100vh-8rem)] space-y-5"
    ...
    >
      <div
        v-if="dragging"
        class="pointer-events-none absolute inset-0 z-30 flex min-h-[28rem] items-center justify-center rounded-xl border-2 border-dashed border-accent bg-canvas/95 p-6 backdrop-blur-sm"
        role="status"
        aria-live="polite"
      >
        <div class="text-center">
          ...
          <p class="mt-1 text-sm text-muted">
            Os arquivos serão adicionados à pasta “{{ currentFolderLabel }}”.
          </p>
        </div>
      </div>
      ...
      <header class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div class="min-w-0">
          <nav aria-label="Caminho da pasta" class="flex flex-wrap items-center gap-1 text-sm">
            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-muted hover:bg-elevated hover:text-foreground"
              :class="{ 'font-medium text-foreground': !currentFolder }"
            ...
            <template v-for="folder in currentPath" :key="folder.id">
              ...
              <button
                type="button"
                class="max-w-48 truncate rounded-md px-2 py-1 text-muted hover:bg-elevated hover:text-foreground"
                :class="{ 'font-medium text-foreground': folder.id === currentFolder?.id }"
                @click="navigateToPath(folder)"
              >
                {{ folder.name }}
              </button>
            </template>
          ...
          <p class="mt-1 text-sm text-muted">
            Organize arquivos em pastas, pesquise em todo o Drive e acompanhe o armazenamento.
          </p>
        </div>
        <div class="flex flex-wrap gap-2">
          <AppButton variant="secondary" @click="openCreateFolder">
            <FolderPlus :size="17" aria-hidden="true" />Nova pasta
          </AppButton>
          <AppButton @click="uploadInput?.click()">
            <Upload :size="17" aria-hidden="true" />Importar arquivos
          </AppButton>
          <input
            ref="uploadInput"
            class="sr-only"
      ...
      <section class="rounded-xl border border-border bg-surface p-5" aria-labelledby="storage-title">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center">
          <span class="flex size-11 shrink-0 items-center justify-center rounded-lg bg-accent/15 text-accent">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-baseline justify-between gap-2">
              <div>
                ...
                <p v-if="storageStatus" class="mt-1 text-sm text-muted">
                  {{ formatBytes(storageStatus.usedBytes) }} de
                  {{ formatBytes(storageStatus.reservedBytes) }} utilizados
                </p>
                <p v-else class="mt-1 text-sm text-muted">
                  {{ statusLoading ? 'Calculando armazenamento…' : 'Informações indisponíveis.' }}
                </p>
              </div>
              <span v-if="storageStatus" class="text-sm font-medium">
                {{ formatBytes(storageStatus.effectiveFreeBytes) }} disponíveis
              </span>
            </div>
            <div class="mt-3 h-2 overflow-hidden rounded-full bg-elevated" aria-hidden="true">
              <div
            ...
            <div v-if="storageStatus" class="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted">
              ...
              <span v-if="physicalDiskLimitsDrive">
                O espaço físico da máquina limita a disponibilidade atual.
              </span>
            </div>
          </div>
        </div>
      ...
      <section v-if="queue.length" class="rounded-xl border border-border bg-surface p-4">
        ...
        <ul class="mt-3 divide-y divide-border">
          <li v-for="item in queue" :key="item.id" class="flex flex-col gap-3 py-3 sm:flex-row sm:items-center">
            <FileText class="shrink-0 text-muted" :size="18" aria-hidden="true" />
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap justify-between gap-2 text-sm">
                <p class="truncate font-medium">{{ item.file.name }}</p>
                <span class="text-xs text-muted">Destino: {{ item.targetFolderName }}</span>
              </div>
              <div class="mt-2 h-1.5 overflow-hidden rounded bg-elevated">
                <div
              ...
              <p class="mt-1 text-xs text-muted">
                {{ item.status }}<span v-if="item.status === 'enviando'"> · {{ item.progress }}%</span>
                <span v-if="item.error"> · {{ item.error }}</span>
              </p>
            </div>
            <AppButton v-if="item.status === 'enviando'" variant="secondary" @click="cancel(item)">
      ...
      <section class="rounded-xl border border-border bg-surface">
        <div class="border-b border-border p-4">
          <div class="flex flex-col gap-3 lg:flex-row lg:items-center">
            <label class="relative min-w-0 flex-1">
              ...
              <input
                v-model="query"
                class="min-h-11 w-full rounded-md border border-border bg-canvas py-2 pl-10 pr-3 text-sm outline-none focus:border-accent focus:ring-2 focus:ring-accent/20"
                placeholder="Buscar arquivos por nome em todo o Drive"
              />
            </label>
            <div class="flex flex-wrap gap-2">
              <AppButton
              ...
              >
                ...
                <span
                  ...
                  class="rounded-full bg-accent px-1.5 py-0.5 text-[10px] leading-none text-white"
                >
              ...
              <label>
                ...
                <select
                  v-model="sort"
                  class="min-h-10 rounded-md border border-border bg-elevated px-3 text-sm"
                >
                  <option value="updatedAt:desc">Mais recentes</option>
                  <option value="updatedAt:asc">Mais antigos</option>
                  <option value="createdAt:desc">Criação recente</option>
              ...
              <div class="flex rounded-md border border-border bg-elevated p-1" aria-label="Modo de exibição">
                <button
                  type="button"
                  class="rounded p-1.5 text-muted hover:text-foreground"
                  :class="{ 'bg-border text-foreground': viewMode === 'list' }"
                ...
                <button
                  type="button"
                  class="rounded p-1.5 text-muted hover:text-foreground"
                  :class="{ 'bg-border text-foreground': viewMode === 'grid' }"
                  aria-label="Exibir como grade"
          ...
          <label v-if="query.trim()" class="mt-3 inline-flex items-center gap-2 text-sm text-muted">
            <input v-model="searchCurrentFolder" type="checkbox" class="size-4 rounded border-border" />
            Buscar somente em “{{ currentFolderLabel }}”
          </label>

          <div
            v-if="showFilters"
            id="drive-filters"
            class="mt-4 grid gap-3 rounded-lg border border-border bg-canvas p-4 sm:grid-cols-2 xl:grid-cols-5"
          >
            <label class="text-xs font-medium text-muted">
              ...
              <input
                ...
                type="date"
                class="mt-1 min-h-10 w-full rounded-md border border-border bg-surface px-2 text-sm text-foreground"
              />
            </label>
            <label class="text-xs font-medium text-muted">
              Modificado até
          ...
          <div v-if="activeFilters.length" class="mt-3 flex flex-wrap items-center gap-2">
            <button
              ...
              :key="item.key"
              type="button"
              class="inline-flex items-center gap-1 rounded-full border border-border bg-elevated px-2.5 py-1 text-xs text-muted hover:text-foreground"
              @click="clearFilter(item.key)"
            >
              {{ item.label }}<X :size="13" aria-hidden="true" />
            </button>
            <button type="button" class="text-xs text-accent hover:underline" @click="clearFilters">
              Limpar filtros
            </button>
          </div>
        </div>

        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3 text-sm text-muted">
          <span>{{ resultLabel }}</span>
          <span v-if="query.trim() && !searchCurrentFolder">Resultados em todo o Drive</span>
          <span v-else>Conteúdo de {{ currentFolderLabel }}</span>
        </div>

        <p
          v-if="error"
          class="m-4 rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-500"
          role="alert"
        >
          {{ error }}
        </p>

        <div v-if="folderLoading || loading" class="p-5 text-sm text-muted">
          Carregando conteúdo…
        </div>
        ...
        <template v-else>
          <section v-if="visibleFolders.length" class="border-b border-border p-4" aria-labelledby="folders-title">
            ...
            <ul class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <li
                ...
                :key="folder.id"
                class="group rounded-lg border border-border bg-canvas p-3 transition hover:border-accent/60"
              >
                <div class="flex items-start gap-3">
                  ...
                  >
                    ...
                    <span class="min-w-0">
                      ...
                      <span class="mt-0.5 block text-xs text-muted">Abrir pasta</span>
                    </span>
                  </button>
                  <div class="flex shrink-0 gap-1 opacity-70 transition group-hover:opacity-100">
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      class="rounded-md p-2 text-muted hover:bg-red-500/10 hover:text-red-500"
          ...
          <div
            v-if="files.length === 0 && visibleFolders.length === 0"
          ...
          >
            ...
            <p class="mt-3 font-medium text-foreground">
              {{ query.trim() || activeFilters.length ? 'Nenhum arquivo encontrado' : 'Esta pasta está vazia' }}
            </p>
            <p class="mt-1">
              {{
                query.trim() || activeFilters.length
                  ? 'Ajuste a busca ou remova alguns filtros.'
                  : 'Crie uma pasta, importe arquivos ou arraste-os para esta área.'
              }}
            </p>
            <div v-if="!query.trim() && !activeFilters.length" class="mt-4 flex justify-center gap-2">
              <AppButton variant="secondary" @click="openCreateFolder">
          ...
          <ul v-else-if="files.length && viewMode === 'list'" class="divide-y divide-border">
            <li v-for="file in files" :key="file.id" class="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-1 text-xs text-muted">
                  <span v-if="file.folderPath && query.trim()">{{ file.folderPath }} · </span>
          ...
          <ul v-else-if="files.length" class="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
            <li
              v-for="file in files"
              :key="file.id"
              class="flex min-h-44 flex-col rounded-lg border border-border bg-canvas p-4"
            >
              ...
              <div class="mt-4 min-w-0 flex-1">
                ...
                <p v-if="file.folderPath && query.trim()" class="mt-1 truncate text-xs text-muted">
                  {{ file.folderPath }}
                </p>
                <p class="mt-1 text-xs text-muted">
                  {{ formatBytes(file.sizeBytes) }} · {{ formatDate(file.updatedAt) }}
        ...
        <div
          v-if="total > DRIVE_PAGE_SIZE"
          class="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3 text-sm text-muted"
        >
          <span>
            Página {{ page + 1 }} de {{ Math.ceil(total / DRIVE_PAGE_SIZE) }}
          </span>
          <div class="flex gap-2">
            ...
            <AppButton
              variant="secondary"
              :disabled="(page + 1) * DRIVE_PAGE_SIZE >= total"
              @click="page += 1"
            >
              Próxima
            </AppButton>
