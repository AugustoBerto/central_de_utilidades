<script setup>
import { BookOpen, ChevronDown, ChevronUp, ExternalLink, Folder, Globe, Link, Pencil, Plus, Server, Shield, Terminal, Trash2, Zap } from 'lucide-vue-next';
...
async function closeModal() {
  if (dirty.value) {
    const discard = await confirm({
      ...
      message: 'As alterações não salvas deste atalho serão perdidas.',
...
async function remove(shortcut) {
  const accepted = await confirm({
    ...
    message: `“${shortcut.label}” será removido da sua lista de acessos.`,
...
<template>
  <AppShell>
    <section class="min-w-0 space-y-6">
      <div class="flex flex-wrap items-start justify-between gap-4"><div><h2 class="font-semibold">Acessos frequentes</h2><p class="mt-1 text-sm text-muted">Abra e organize sistemas externos com segurança.</p></div><AppButton @click="openCreate"><Plus :size="17" aria-hidden="true" />Novo atalho</AppButton></div>
      <p v-if="error && !modalOpen" class="rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-500" role="alert">{{ error }}</p>
      ...
      <div v-else-if="shortcuts.length === 0" class="rounded-lg border border-border bg-surface p-10 text-center text-sm text-muted"><Link class="mx-auto" :size="28" aria-hidden="true" /><p class="mt-3">Nenhum atalho criado ainda.</p><AppButton class="mt-4" @click="openCreate"><Plus :size="17" />Criar primeiro atalho</AppButton></div>
      <template v-else>
        <section v-for="[group, items] in groups" :key="group" class="space-y-3">
          ...
          <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <article v-for="(shortcut, index) in items" :key="shortcut.id" class="flex min-w-0 flex-col rounded-lg border border-border bg-surface p-4 transition-colors hover:border-accent/60">
              <div class="flex gap-3"><span class="rounded-md border border-border bg-elevated p-2 text-accent"><component :is="icons[shortcut.iconKey]" :size="20" aria-hidden="true" /></span><div class="min-w-0 flex-1"><div class="flex flex-wrap items-center gap-2"><h4 class="break-words font-medium">{{ shortcut.label }}</h4><span v-if="shortcut.isPinned" class="rounded bg-accent/15 px-1.5 py-0.5 text-xs text-accent">Fixado</span></div><p class="mt-1 truncate text-xs text-muted" :title="shortcut.url">{{ shortcut.url }}</p></div></div>
              <div class="mt-auto flex flex-wrap items-center gap-2 pt-5"><a :href="shortcut.url" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-md border border-accent bg-accent px-3 text-sm font-medium text-white hover:opacity-90">Abrir <ExternalLink :size="15" aria-hidden="true" /></a><AppButton variant="secondary" title="Editar" @click.stop="edit(shortcut)"><Pencil :size="15" /><span class="sr-only">Editar {{ shortcut.label }}</span></AppButton><AppButton variant="secondary" title="Mover para cima" :disabled="index === 0 || movingId !== null" @click.stop="move(shortcut, -1)"><ChevronUp :size="15" /><span class="sr-only">Mover para cima</span></AppButton><AppButton variant="secondary" title="Mover para baixo" :disabled="index === items.length - 1 || movingId !== null" @click.stop="move(shortcut, 1)"><ChevronDown :size="15" /><span class="sr-only">Mover para baixo</span></AppButton><AppButton variant="danger" title="Excluir" @click.stop="remove(shortcut)"><Trash2 :size="15" /><span class="sr-only">Excluir {{ shortcut.label }}</span></AppButton></div>
            </article>
          </div>
        </section>
      </template>
    ...
    <AppModal v-if="modalOpen" :title="modalTitle" max-width-class="max-w-xl" @close="closeModal">
      <form class="space-y-4 p-5 sm:p-6" @submit.prevent="save">
        <label class="block text-sm"><span>Rótulo</span><input v-model="form.label" class="mt-1 w-full rounded-md border border-border bg-canvas px-3 py-2" :aria-invalid="Boolean(fieldErrors.label)" maxlength="80" required><span v-if="fieldErrors.label" class="mt-1 block text-xs text-red-500">{{ fieldErrors.label }}</span></label>
        <label class="block text-sm"><span>URL HTTPS</span><input v-model="form.url" class="mt-1 w-full rounded-md border border-border bg-canvas px-3 py-2" type="url" :aria-invalid="Boolean(fieldErrors.url)" placeholder="https://" required><span v-if="fieldErrors.url" class="mt-1 block text-xs text-red-500">{{ fieldErrors.url }}</span></label>
        <label class="block text-sm"><span>Grupo <span class="text-muted">(opcional)</span></span><input v-model="form.groupName" class="mt-1 w-full rounded-md border border-border bg-canvas px-3 py-2" maxlength="80"></label>
        <label class="block text-sm"><span>Ícone</span><select v-model="form.iconKey" class="mt-1 min-h-10 w-full rounded-md border border-border bg-canvas px-3"><option v-for="(_, key) in icons" :key="key" :value="key">{{ key }}</option></select></label>
