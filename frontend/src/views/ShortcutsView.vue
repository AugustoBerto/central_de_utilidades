...
const editingShortcut = computed(() => shortcuts.value.find((shortcut) => shortcut.id === editingId.value) ?? null);
const pinnedCount = computed(() => shortcuts.value.filter((shortcut) => shortcut.isPinned).length);
const pinLimitReached = computed(() => pinnedCount.value >= 5 && !editingShortcut.value?.isPinned);
  ...
  <AppShell>
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-sm text-muted">
            As setas organizam apenas esta página. A barra superior possui {{ pinnedCount }} de 5 posições ocupadas.
          </p>
        </div>
              ...
              <div class="flex gap-3">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span v-if="shortcut.isPinned" class="rounded bg-accent/15 px-1.5 py-0.5 text-xs text-accent" :title="`Posição ${shortcut.pinnedSlot + 1} na barra superior`">Fixado</span>
                  </div>
                  <p class="mt-1 truncate text-xs text-muted" :title="shortcut.url">{{ shortcut.url }}</p>
                </div>
              </div>
              <div class="mt-auto flex flex-wrap items-center gap-2 pt-5">
                <a :href="shortcut.url" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-md border border-accent bg-accent px-3 text-sm font-medium text-white hover:opacity-90">Abrir <ExternalLink :size="15" aria-hidden="true" /></a>
                <AppButton variant="secondary" title="Editar" @click.stop="edit(shortcut)"><Pencil :size="15" /><span class="sr-only">Editar {{ shortcut.label }}</span></AppButton>
                <AppButton variant="secondary" title="Mover para cima nesta página" :disabled="index === 0 || movingId !== null" @click.stop="move(shortcut, -1)"><ChevronUp :size="15" /><span class="sr-only">Mover para cima nesta página</span></AppButton>
                <AppButton variant="secondary" title="Mover para baixo nesta página" :disabled="index === items.length - 1 || movingId !== null" @click.stop="move(shortcut, 1)"><ChevronDown :size="15" /><span class="sr-only">Mover para baixo nesta página</span></AppButton>
    ...
    <AppModal v-if="modalOpen" :title="modalTitle" max-width-class="max-w-xl" @close="closeModal">
      <form class="space-y-4 p-5 sm:p-6" @submit.prevent="save">
        ...
        <div class="rounded-md border border-border bg-canvas/60 p-3">
          <label class="flex items-center gap-2 text-sm">
            <input v-model="form.isPinned" type="checkbox" :disabled="pinLimitReached && !form.isPinned">
            Fixar para acesso rápido
          </label>
          <p class="mt-2 text-xs text-muted">
            {{ pinnedCount }} de 5 posições ocupadas. Novos itens entram automaticamente da direita para a esquerda e podem ser arrastados na barra superior.
          </p>
          <p v-if="pinLimitReached && !form.isPinned" class="mt-2 text-xs text-red-500">
            Desafixe um atalho existente para liberar uma posição.
          </p>
        </div>
